...
const BlogCard = ({blog}) => {
    ...
    const {title, decription, category, image, id} = blog;

    const navigate = useNavigate();

    return (
        <div onClick={()=>navigate(`/blog/${id}`)} className='w-full rounded-lg
        ...
        cursor-pointer'>
            ...
            <div className='p-5'>
                ...
                <p className='mb-3 text-xs text-gray-600'>{decription.slice(0,80)}</p>
            </div>
        </div>
    )
}
